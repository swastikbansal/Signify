...
class _Voicetosign1WidgetState extends State<Voicetosign1Widget> with RouteAware, SingleTickerProviderStateMixin {
    ...
    if (inputSentence.isEmpty) {
      ...
      setState(() {
        ...
        if (matches.isEmpty) {
          ...
          currentWord = wordToAnimationMap.entries.firstWhere((entry) => entry.value == matches[0]).key;
        }
      });
  ...
  void _playNextAnimation() {
    ...
    _fadeController.reverse().then((_) {
      setState(() {
        currentAnimation = animationQueue.removeAt(0); // Get and remove the first animation
        currentWord = wordToAnimationMap.entries.firstWhere((entry) => entry.value == currentAnimation, orElse: () => MapEntry('', defaultAnimation)).key;
      });
      ...
      animationTimer = Timer(Duration(seconds: duration), () {
        if (animationQueue.isNotEmpty) {
          ...
          setState(() {
            currentAnimation = defaultAnimation; // Show default animation when done
            currentWord = null;
          });
        }
      });
                            ...
                            builder: (context, child) {
                              return Opacity(
                                ...
                                child: currentAnimation != null
                                  ...
                                  backgroundColor: FlutterFlowTheme.of(context).primaryBackground,
                                  cameraTarget: '0m 1.5m 0m', // Keeps the avatar centered
                                  cameraOrbit: '0deg 75deg 2.5m', // Adjust camera orbit for positioning
                                )
                                    : const Center(
                                  child: Text(
